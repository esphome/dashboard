...
class ESPHomeMainView extends LitElement {
  ...
  private _handleViewModeChanged(e: CustomEvent) {
    ...
    if (devicesList) {
      devicesList.dispatchEvent(new CustomEvent('view-mode-changed', { detail: e.detail }));
    }
  }
  ...
  private _handleSortChanged(e: CustomEvent) {
    ...
    const devicesList = document.querySelector('esphome-devices-list');
    if (devicesList) {
      devicesList.dispatchEvent(new CustomEvent('sort-changed', { detail: e.detail }));
    }
  }
  ...
  private _handleFilterChanged(e: CustomEvent) {
    ...
    const devicesList = document.querySelector('esphome-devices-list');
    if (devicesList) {
      devicesList.dispatchEvent(new CustomEvent('filter-changed', { detail: e.detail }));
    }
  }
}
