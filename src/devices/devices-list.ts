...
class ESPHomeDevicesList extends LitElement {
  ...
  protected render() {
    const emptySvg = html`
      <svg fill="currentColor" viewBox="0 0 24 24" class="empty-icon" xmlns="http://www.w3.org/2000/svg">
        <path
    ...
    if (this._devices?.length === 0) {
      return html`
        <section class="no-result-container">
          ...
          <p class="helper-text">It looks like you don't have any devices yet.</p>
          <mwc-button raised label="New device" icon="add" @click=${this._handleOpenWizardClick}></mwc-button>
        </section>
      `;
    }
    ...
    if (!this._devices) {
      return html` <div class="loading-container">
        <div class="grid">
          <esphome-importable-device-card skeleton></esphome-importable-device-card>
        </div>
      </div>`;
    }
  ...
  private _filter(item: ImportableDevice | ConfiguredDevice): boolean {
    if (this._search?.value) {
      ...
      if ("comment" in item && item.comment && item.comment!.toLowerCase().indexOf(searchValue) >= 0) {
        return true;
      }
      if ("project_name" in item && item.project_name && item.project_name!.toLowerCase().indexOf(searchValue) >= 0) {
        return true;
      }
      return false;
    }
