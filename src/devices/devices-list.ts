...
class ESPHomeDevicesList extends LitElement {
  ...
  private _renderStatus(row: DataTableRowData): TemplateResult {
    if (row.type === "importable") {
      return html`<span class="status-badge discovered">Discovered</span>`;
    }

    const isOnline = this._onlineStatus[row.configuration];
    return html`<span class="status-badge ${isOnline ? "online" : "offline"}">
  ...
  private _getTableData(): DataTableRowData[] {
    ...
    return devices.map((device) => ({
      ...
      status_sort: this._isImportable(device)
        ? 2 // Discovered devices sort last
        : this._onlineStatus[(device as ConfiguredDevice).configuration]
          ? 0
          : 1, // Online first, then offline
      ip_sort: this._isImportable(device)
        ? device.network || ""
        : this._deviceIPs[device.name] ||
          (device as ConfiguredDevice).address ||
          "",
      name_sort: device.friendly_name || device.name,
    }));
  }
  ...
  protected render() {
    ...
    const filteredData = tableData.filter((item) => this._filter(item));
  ...
  private _getSortDirection(): "asc" | "desc" | null {
    if (this._sortBy === "status") {
      return "asc"; // Online first (0), then offline (1), then discovered (2)
    }
    return "asc";
  }
  ...
  static styles = css`
    ...
    .status-indicator.offline {
      ...
      box-shadow: 0 0 4px var(--alert-error-color, #d93025);
    }
    .status-indicator.discovered {
      background-color: var(--alert-info-color, #00539f);
      box-shadow: 0 0 4px var(--alert-info-color, #00539f);
    }
    .status-badge {
      display: inline-flex;
      align-items: center;
    ...
    .status-badge.discovered {
      background-color: var(--alert-info-color-bg, #e6eef5);
      color: var(--alert-info-color, #00539f);
    }
    .actions-container {
      display: flex;
      gap: 8px;
